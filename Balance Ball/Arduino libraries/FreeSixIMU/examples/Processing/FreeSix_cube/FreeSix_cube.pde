...
final String serialPort = "/dev/cu.usbserial-A104OJGA"; // replace this with your serial port. On windows you will need something like "COM1".

float [] q = new float [4];
float [] hq = null;
float [] Euler = new float [3]; // psi, theta, phi

int lf = 10; // 10 is 'n' in ASCII
...
void draw() {
  ...
  textFont(font, 20);
  textAlign(LEFT, TOP);
  text("Q:n" + q[0] + "n" + q[1] + "n" + q[2] + "n" + q[3], 20, 20);
  text("Euler Angles:nYaw (psi)  : " + degrees(Euler[0]) + "nPitch (theta): " + degrees(Euler[1]) + "nRoll (phi)  : " + degrees(Euler[2]), 200, 20);
